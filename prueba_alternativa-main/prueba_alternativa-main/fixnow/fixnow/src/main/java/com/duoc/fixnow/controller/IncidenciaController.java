...
public class IncidenciaController {
    ...
    @PutMapping("/actIncidente/{id}")
    public ResponseEntity<?> actualizarIncidente(@PathVariable Integer id, @RequestBody Incidencia cuerpo){
        try {
            if (id == null){
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Debe ingresar un id para buscar un incidente");}
            if(cuerpo.getTitulo() == null|| cuerpo.getDescripcion()== null|| cuerpo.getEstado()== null||
                    cuerpo.getPrioridad()== null||cuerpo.getUsuarioReportante()== null||cuerpo.getFechaRegistro() == null){
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Se deben ingresar todos los datos para ingresar un incidente");
            }
            return ResponseEntity.ok(incidenciaService.updateIncidencia(id,cuerpo));
        }catch (Exception e){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("No se pudo actualizar el incidente");
        }
    }
}
